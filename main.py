...
'''with tab_time:
    ...
    if aggregation_type == "cuisine_type":
        df1_filtrado=df1_filtrado.query("cuisine_type in @item_or_cuisine_type_new_customer")
        df2_filtrado=df2_filtrado.query("cuisine_type in @item_or_cuisine_type_new_customer")
        new_customers_top = df2_filtrado[["cuisine_type","first_time_orders"]].groupby(["cuisine_type"]).agg("sum").reset_index(drop=False).sort_values(by="first_time_orders")

    else:
        df1_filtrado=df1_filtrado.query("item_type in @item_or_cuisine_type_new_customer")
        df2_filtrado=df2_filtrado.query("item_type in @item_or_cuisine_type_new_customer")
        new_customers_top = df2_filtrado[["item_type","first_time_orders"]].groupby(["item_type"]).agg("sum").reset_index(drop=False).sort_values(by="first_time_orders")
    ...
    col1_b.markdown("<h3>New Customers</h3>", unsafe_allow_html=True)
    col1_b.metric("New Customers", millify(new_customers, precision=2), delta=str(round(delta_new_costumers,2))+"%")
    col3_b.plotly_chart(fig,use_container_width = True)
    col4_b.plotly_chart(fig2,use_container_width = True)
    
    ### COMPLETED ORDERS
    
    col5_b, col6_b = st.columns(2)      
    col7_b, col8_b = st.columns(2)
    ...
    col5_b.markdown("<h3>Completed orders</h3>", unsafe_allow_html=True)
    col5_b.metric("", millify(orders, precision=2), delta=str(round(delta_orders,2))+"%")
    
    col7_b.plotly_chart(fig,use_container_width = True)
    col8_b.plotly_chart(fig2,use_container_width = True)'''
    
#else:
#    df1_filtrado=df1_filtrado.query("item_type in @item_or_cuisine_type")
#    df2_filtrado=df2_filtrado.query("item_type in @item_or_cuisine_type")
#    
#    new_customers = df2_filtrado["first_time_orders"].sum()
#    delta_new_costumers = (( df2_filtrado["first_time_orders"].sum() - df1_filtrado["first_time_orders"].sum() ) / df1_filtrado["first_time_orders"].sum())*100    
#    col1.metric("New Customers", millify(new_customers, precision=2), delta=str(round(delta_new_costumers,2))+"%")

#print(new_customers)
#print(df1_filtrado["first_time_orders"].sum())
#print(delta_new_costumers)
#print(aov)
#print(aov_old)       
